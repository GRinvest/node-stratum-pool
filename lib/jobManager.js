...
// random unique job per new block template
var JobCounter = function () {
    var counter = 0x0004311CA7;
    this.next = function () {
        counter += randomInt(256, 2048);
        if (counter % 0xffffffffff === 0)
            counter = randomInt(1, 256);
        
        return this.cur();
    };
    this.cur = function () {
        return util.sha256d(counter.toString(16)).toString('hex').substr(randomInt(0, 16),8);
    };
};
...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln, software) {
        ...
        //0-9A-Fa-f only
        if(/^[\x30-\x39\x41-\x46\x61-\x66]+$/g.test(nTime) == false) {
            return shareError([20, 'invalid hex in nTime']);
        }
        //0-9A-Fa-f only
        if(/^[\x30-\x39\x41-\x46\x61-\x66]+$/g.test(extraNonce1) == false) {
            return shareError([20, 'invalid hex in extraNonce']);
        }
        //0-9A-Fa-f only
        if(/^[\x30-\x39\x41-\x46\x61-\x66]+$/g.test(extraNonce2) == false) {
            return shareError([20, 'invalid hex in extraNonce2']);
        }
        //0-9A-Fa-f only
        if(/^[\x30-\x39\x41-\x46\x61-\x66]+$/g.test(soln) == false) {
            return shareError([20, 'invalid hex in solution']);
        }
        ...
        // improved duplicate share detection
        var hdr = extraNonce1.toLowerCase() + extraNonce2.toLowerCase();
        var sol = soln.slice(6).substr(0, 32).toLowerCase();
        if (!job.registerSubmit(hdr,sol)) {
        ...
        */
        ...
        // notify pool.js of valid share for further processing
        _this.emit('share', {
            job: jobId,
            ip: ipAddress,
            port: port,
